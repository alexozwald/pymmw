...
def orjson_to_df(filepath: str|list[str]) -> pd.DataFrame:
    ...
    df = pd.DataFrame()
    filepath = list(filepath) if isinstance(filepath, str) else filepath
...
''' MAYAVI VIZUALIZATIONS
#mlab.init_notebook()

def mlab_anim_3d_v1(df: pd.DataFrame, ANIM_COL: str = 'frame'):
    ...
    for frame in anim_frames:
        mlab.clf()
        frame_data = df[df[ANIM_COL] == frame]

        x = frame_data['x'].values
        y = frame_data['y'].values
        z = frame_data['z'].values
        v = frame_data['v'].values

        s = mlab.points3d(x, y, z, v, colormap='viridis', scale_mode='none', scale_factor=0.5)
        #s.module_manager.scalar_lut_manager.title.text = 'Velocity'

        mlab.view(azimuth=45, elevation=45, distance=60)
        mlab.axes(ranges=(*max_bounds['x'], *max_bounds['y'], *max_bounds['z']),
                  xlabel='X', ylabel='Y', zlabel='Z')

        mlab.draw()
        sleep(0.01)

def mlab_anim_3d_v2(df: pd.DataFrame, ANIM_COL: str = 'frame'):
    anim_frames = np.sort(df[ANIM_COL].unique())
    max_bounds = {'x': (df['x'].min(), df['x'].max()),
                  'y': (df['y'].min(), df['y'].max()),
                  'z': (df['z'].min(), df['z'].max())}
...
def mlab_anim_3d_v3(df: pd.DataFrame, ANIM_COL: str = 'frame'):
    anim_frames = np.sort(df[ANIM_COL].unique())
    max_bounds = {'x': (df['x'].min(), df['x'].max()),
                  'y': (df['y'].min(), df['y'].max()),
                  'z': (df['z'].min(), df['z'].max())}
...
def mpl_anim_3d_v4(df: pd.DataFrame, ANIM_COL: str = 'stack'):
    anim_frames = np.sort(df[ANIM_COL].unique())
    max_bounds = {'x': (df['x'].min(), df['x'].max()),
                  'y': (df['y'].min(), df['y'].max()),
                  'z': (df['z'].min(), df['z'].max())}
    ...
    def update(frame):
        ...
        sc = ax.scatter(x, y, z, c=v, cmap='viridis', s=20, marker='o', edgecolor='k', alpha=0.8)
    ...
    ani = animation.FuncAnimation(fig, update, frames=anim_frames, interval=50, blit=True)

    return ani

def mpl_anim_3d_v5(df: pd.DataFrame, ANIM_COL: str = 'stack', interval=0.05):
    anim_frames = np.sort(df[ANIM_COL].unique())
    max_bounds = {'x': (df['x'].min(), df['x'].max()),
                  'y': (df['y'].min(), df['y'].max()),
                  'z': (df['z'].min(), df['z'].max())}
...
def mpl_anim_3d_v6(df: pd.DataFrame, ANIM_COL: str = 'stack', interval=0.05):
    anim_frames = np.sort(df[ANIM_COL].unique())
    max_bounds = {'x': (df['x'].min(), df['x'].max()),
                  'y': (df['y'].min(), df['y'].max()),
                  'z': (df['z'].min(), df['z'].max())}
...
def mpl_anim_3d_v7(df: pd.DataFrame, ANIM_COL: str = 'stack', interval=0.01):
    anim_frames = np.sort(df[ANIM_COL].unique())
    max_bounds = {'x': (df['x'].min(), df['x'].max()),
                  'y': (df['y'].min(), df['y'].max()),
                  'z': (df['z'].min(), df['z'].max())}
    ...
    sm.set_array([])
    cbar = fig.colorbar(sm, ax=ax, pad=0.1)
    cbar.ax.set_title('Velocity (m/s)')
    ...
    for frame in anim_frames:
        ...
        #normalized_opacity = ((frame_data['frame'] - min_frame) / (max_frame - min_frame)) * (max_opacity - min_opacity) + min_opacity
        ...
        sc = ax.scatter(x, y, z, c=v, cmap='viridis', s=20, marker='o', edgecolor='k', alpha=normalized_opacity)
...
def mpl_anim_3d_v8(df: pd.DataFrame, ANIM_COL: str = 'stack', interval=0.01, COLOR1_COL: str = 'v', COLOR1_TITLE: str = 'Velocity (m/s)'):
...
def mpl_anim_3d_v9(df: pd.DataFrame, ANIM_COL: str = 'stack', COLOR1_COL: str = 'v', COLOR1_TITLE: str = 'Velocity (m/s)'):
    anim_frames = np.sort(df[ANIM_COL].unique())
    max_bounds = {'x': (df['x'].min(), df['x'].max()),
                  'y': (df['y'].min(), df['y'].max()),
                  'z': (df['z'].min(), df['z'].max())}
    ...
    slider = Slider(slider_ax, 'Stack', anim_frames[0], anim_frames[-1], valinit=anim_frames[0], valstep=1)
    ...
    def update(val):
        ...
        sc = ax.scatter(x, y, z, c=v, cmap='viridis', s=20, marker='o', edgecolor='k', linewidths=0.6, alpha=1)
    ...
    # Update the plot with the initial slider value
    update(slider.val)

    plt.show()

def mpl_anim_3d_v10(df: pd.DataFrame,
                   ANIM_COL: str = 'stack',
                   COLOR1_COL: str = 'v',
                   COLOR1_TITLE: str = None,
                   COLOR2_COL: str = 'clstr',
    ...
    print(f"COLOR1_COL: {COLOR1_COL if color1_valid else 'None':>10}    COLOR1_TITLE: {COLOR1_TITLE if (color1_valid and COLOR1_TITLE) else 'None':>16}    len(df)={len(df)}")
    print(f"COLOR2_COL: {COLOR2_COL if color2_valid else 'None':>10}    COLOR2_TITLE: {COLOR2_TITLE if (color2_valid and COLOR2_TITLE) else 'None':>16}    ANIM_COL={ANIM_COL}")
    ...
    if color1_valid and color2_valid:
        ...
        update_fn = lambda val: (update(val, ax1, COLOR1_COL, COLOR1_TITLE or COLOR1_COL), update(val, ax2, COLOR2_COL, COLOR2_TITLE or COLOR2_COL))
    elif color1_valid:
        ...
        print(f"Displaying column {COLOR2_COL} on a single subplot")
        ax1 = fig.add_subplot(111, projection='3d')
        update_fn = lambda val: update(val, ax1, COLOR2_COL, COLOR2_TITLE or COLOR2_COL)
...
def mpl_anim_3d_v11(df: pd.DataFrame, ANIM_COL: str = 'stack', COLOR1_COL: str = 'v', COLOR1_TITLE: str = None,
    ...
    def update(val, ax, color_col, color_title):
        ...
        sc = ax.scatter(x, y, z, c=c, cmap='viridis', s=20, marker='o', edgecolor='k', linewidths=0.6, alpha=normalized_opacity)
        ax.set_xlabel('X')
        ax.set_ylabel('Y')
        ax.set_zlabel('Z')
        ax.set_xlim(*max_bounds['x'])
    ...
    if color1_valid and color2_valid:
        ...
        sm1 = plt.cm.ScalarMappable(cmap='viridis', norm=plt.Normalize(vmin=c1_min, vmax=c1_max))
        sm1.set_array([])
        ...
        sm2 = plt.cm.ScalarMappable(cmap='viridis', norm=plt.Normalize(vmin=c2_min, vmax=c2_max))
        sm2.set_array([])
...
viz_time = mpl_anim_3d_v8
viz_slider = mpl_anim_3d_v11
...
if __name__ == "__main__":
    ...
    #ani = mpl_anim_3d_v10(df, ANIM_COL='stack', COLOR1_COL='frame', COLOR2_COL='v')
    #ani = mpl_anim_3d_v11(df, ANIM_COL='stack', COLOR1_COL='frame', COLOR2_COL='v', OPACITY_COL='frame')
    #ani = mpl_anim_3d_v11(df, 'stack')
    #mlab_anim_3d_v1(df[:10_000], 'frame')
    #mlab_anim_3d_v2(df[:10_000], 'frame')
    #fig = mlab_anim_3d_v3(df[:10_000], 'frame')
    #mlab.show()
    #ani = mpl_anim_3d_v4(df[:200_000], 'stack')
    #plt.show()
    #ani = mpl_anim_3d_v5(df[:200_000], 'stack')
    #ani = mpl_anim_3d_v6(df[:200_000], 'stack')
    #ani = mpl_anim_3d_v7(df[:200_000], 'stack')
    #ani = mpl_anim_3d_v8(df[:200_000], 'stack')
    #ani = mpl_anim_3d_v9(df, 'stack')

    #ani = viz_slider(df, COLOR1_COL='v', COLOR1_TITLE='Velocity (m/s)', COLOR2_COL='frame', COLOR2_TITLE='Frame #', OPACITY_COL='frame')
