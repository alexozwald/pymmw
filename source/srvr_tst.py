...
class Plot3Dv:
    ...
    def add_pts(self, df_new: pd.DataFrame):
        ...
        self.df = pd.concat([self.df, df_new], ignore_index=False)
...
'''
class ZmqListener:
    """ Listener class for ZMQ Port (async-enabled) """
    ...
    async def listen(self) -> Generator[pd.DataFrame, None, None]:
        while True:
            ...
            if ('file' in msg_dict) and ('dir' in msg_dict):
                df_line = df_line.assign(file=msg_dict['file'], dir=msg_dict['dir'])
...
class ZmqListener:
    ...
    def listen(self) -> pd.DataFrame:
        while True:
            ...
            # add stop condition /
            if message == b'END_RUN':
                yield 'STOP'

            msg_dict = orjson.loads(message)
            df_line = pd.DataFrame(msg_dict['xyzv'], columns=['x', 'y', 'z', 'v']).assign(ts=msg_dict['ts'], frame=msg_dict['frame'], file=msg_dict['file'], dir=msg_dict['dir'])
            # currently disabling pyarrow backend bc of weird error
            # <EOL>.convert_dtypes(dtype_backend='pyarrow', inplace=True)
            yield df_line
...
class DataAnalysis:
    ...
    def handle_detections(self, detects: pd.DataFrame, curr_df: pd.DataFrame):
        ...
        # add data archiving
        curr_df['dist'] = (curr_df['x'].pow(2) + curr_df['y'].pow(2) + curr_df['z'].pow(2)).pow(0.5)
        curr_df = curr_df.assign(sent=False)
        if curr_df['stack'].max() % 250 == 0 and len(self.archive) > 10:
            #self.log(f"archive :: ...\n{self.archive.describe(include='all')}")
            pass

        # return if no potential detections
        if self.verbose > 0 and len(detects) == 0:
            self.archive = pd.concat([self.archive, curr_df], ignore_index=True)
            return
        ...
        #curr_df.loc[mask, 'bball'] = True 
        #curr_df.loc[mask, 'sent'] = False
        #curr_df.at[bball_v.index[0],'sent'] = True

        # send point to server & log (always)
        bball_v_ts = pd.Timestamp(bball_v['ts'], unit='ms')
        if bball_v['ts'] not in self.sent_bbals.values:
            idx_sent = self.archive.loc[(self.archive['uuid'] == bball_v['uuid']) & (self.archive['stack'] == bball_v['stack'])].index[0]
            self.archive.at[idx_sent, 'sent'] = True

            bball_v['mph'] = bball_v['v'] / 2.23693629
            tprint(f"=({self.time})==(arch={len(self.archive)})==> BBall Found! v={bball_v['v']:>2.3f} m/s ({bball_v['mph']:>2.1f} mph) @ d={bball_v['dist']:<2.2f}m & ර={bball_v['outlier']:.3f}० @ ({bball_v['x']:>2.1f} {bball_v['y']:>2.1f} {bball_v['z']:>2.1f})m @ {bball_v_ts.strftime('%X')} & □ ={bball_v['frame'].astype(int):<3} {len(detects):>2}=>1 pts")
    ...
    def log(self, s: str, ts: str = None):
        if not ts:                          ts = self.time
        elif isinstance(ts, pd.Timestamp):  ts = ts.strftime('%X')
        tprint(f"{ts} | {s}")
    ...
    def exec_df_get_bball(self, buffer_data: pd.DataFrame):
        ...
        if len(buffer_data) == 0:
            with self._thread_lock:
                ...
                return (self.tmp_detections.copy(deep=True), pd.DataFrame())

        # compute uuids @ start
        buffer_data['uuid'] = pd.util.hash_pandas_object(buffer_data, index=False)
        ...
        # extra stack & split-stuff
        pos_v_mask = self.df.index.isin(self.df.loc[self.df['v'] >= 0].index)
        neg_v_mask = self.df.index.isin(self.df.loc[self.df['v'] < 0].index)
        self.df = self.df.assign(stack=self.stack)
        self.stack += 1

        if (self.verbose > 1) or (self.stack % 500 == 0):
            ...
            arch_len, arch_NA = ( len(self.archive), self.archive.transform('isna',axis=1).sum().sum() )
            self.log(f"□ ={buffer_max:<5} - □ Range={curr_fr_len:<2} - □ skipped={skipped_frames:<1} - □ jumped={frame_jump:<1} - #pts={len_curr_stack:<3} - ARCH={arch_len} & {arch_NA} NA.")

        dfA = self.do_hdbscan(self.df.loc[pos_v_mask], self.MIN_CLSTR, self.ALPHA, norm=self.NORM)
        dfB = self.do_hdbscan(self.df.loc[neg_v_mask], self.MIN_CLSTR, self.ALPHA, norm=self.NORM)
        df = pd.concat([dfA, dfB], ignore_index=False).assign(bball=False)

        # filter bballs
        bball = df.loc[(df['v'] >= self.MPS) & (df['outlier'] >= self.OUT_SCORE)]
        if len(bball) > 0:
            df.loc[df.index.isin(bball.index), 'bball'] = True
            #bball = bball.loc[(bball['v'] == bball['v'].max())]
            #bball = bball.iloc[0:]
            with self._thread_lock:
        ...
        with self._thread_lock:
            return (self.tmp_detections.copy(deep=True), df.copy(deep=True))
...
def main(mps: float, outlier_score: float, normalize: bool, alpha: float, min_cluster_size: int, frame_stack_size: int, min_frames_ball2ball: int, plot: bool, frame_lag: int, internet: bool, verbose: int, **kwargs):
    ...
    handle_executor = DebugThreadPoolExecutor(max_workers=1, non_blocking=False, verbose=verbose) #True
    ...
    if verbose > 0:
        tprint(f"=({data_analysis.time})==> Entering loop...", pd.Timestamp.now())

    for df_line in zmq_listener.listen():

        if isinstance(df_line, str) and df_line == 'STOP':
            data_analysis.log("Beginning csv write! ...", pd.Timestamp.now())
            data_analysis.archive.to_csv("./data.csv", index=False)
            data_analysis.log(f"All done!", pd.Timestamp.now())
            break
        else:
        ...
        if analysis_future and analysis_future.done():
            AF_tmp_detections, AF_df = analysis_future.result()
            handle_executor.submit(data_analysis.handle_detections, AF_tmp_detections, AF_df)
